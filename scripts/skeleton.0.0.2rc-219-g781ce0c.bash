...
make_context() {
	...
	export REPO_URL
	REPO_URL=$(cat <<- 'EOF'
		https://github.com/coherent-oss/cobot
	EOF
	)
	export COVERAGE_URL
	COVERAGE_URL=$(cat <<- 'EOF'
		https://coverage-badge.samuelcolvin.workers.dev/redirect/coherent-oss/cobot
	EOF
	)
	export DOCS_URL
	DOCS_URL=$(cat <<- 'EOF'
		https://cobot.readthedocs.io/en/latest/
	EOF
	)
	export PYPI_URL
	PYPI_URL=$(cat <<- 'EOF'
		https://pypi.org/project/cobot/
	EOF
	)
	export TIDELIFT_URL
	TIDELIFT_URL=$(cat <<- 'EOF'
		https://tidelift.com/subscription/pkg/pypi-cobot?utm_source=pypi-cobot
	EOF
	)
	export SKELETON
	SKELETON=$(cat <<- 'EOF'
	...
	GH_REPO_ARGS=$(cat <<- 'EOF'
		"coherent-oss/cobot" --public --source=./ --remote=upstream --description="The Discord bot for Coherent OSS."
	EOF
	)
	export GH_ENSURE_ENV
	GH_ENSURE_ENV=$(cat <<- 'EOF'
		jq -n '{"deployment_branch_policy": {"protected_branches": false,"custom_branch_policies": true}}' | gh api -H "Accept: application/vnd.github+json" -X PUT "/repos/coherent-oss/cobot/environments/$1" --silent --input -
	EOF
	)
